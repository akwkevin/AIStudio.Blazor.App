...
namespace AIStudio.Business.Quartz_Manage
{
    public class Quartz_TaskBusiness : BaseBusiness<Quartz_Task>, IQuartz_TaskBusiness, ITransientDependency
    {
        ...
        public override async Task AddDataAsync(Quartz_Task data)
        {
            ...
            var result = await AddJob(jobType, data);
            if (result.Success)
            {
        ...
        public Type GetJobClass(string name)
        {
        ...
        public async Task StartAllAsync()
        {
            ...
            foreach (var item in list)
            {
                var jobType = Type.GetType(item.ActionClass);
                if (jobType == null)
                {
                    continue;
                }
        ...
        {
            var jobInfo = new JobInfo(job.TaskName, job.GroupName);
